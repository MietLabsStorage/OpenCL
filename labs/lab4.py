...
def by_buffers():
    ...
    """
    blur, box_blur_time = process_buffer(box_blur_code, data, box_blur['kernel'], np.int32(3))
...
def by___pipes():
    ...
    pipe = clp.get_rw_pipe(ctx, 3, 4096)
    blurred_and_grey_data, blurred_and_grey_time = process_buffer(blur_and_grey_text,
                                                                  ...
                                                                  ctx=ctx,
                                                                  queue=queue)
    cv2.imwrite("by_pipe.jpg", blurred_and_grey_data)
